# Task 23B2 — PDF cross-page table runtime verify
...
$OutJson = Join-Path $PSScriptRoot "_run_23b2_results.json"
$Entities = @(
...
$results.qdrantCollections = $qc.result.collections.Count
...
# Parser logs (docker writes to stderr — do not treat as terminating error)
Start-Sleep -Seconds 2
$prevEap = $ErrorActionPreference
$ErrorActionPreference = "Continue"
$mergeRaw = cmd /c "docker logs chatbot-backend --since 300s 2>&1"
$ErrorActionPreference = $prevEap
$mergeLogs = @($mergeRaw | Select-String -Pattern "Table MERGED continuation|REPEATED_HEADER|DATA_ROWS")
$results.parserMergeLogLines = @($mergeLogs | ForEach-Object { $_.Line })
$results.parserMergeSeen = ($mergeLogs.Count -gt 0)

# MySQL chunks
$docBin = $docId.Replace("-", "")
$sqlChunks = @(cmd /c "docker exec ragchatbot-mysql mysql -uroot -proot ragchatbot -N -e `"SELECT chunk_type, LEFT(content, 120) FROM document_chunks WHERE document_id = UNHEX('$docBin') AND deleted_at IS NULL ORDER BY chunk_index LIMIT 50;`" 2>&1")
...
$results.sqlChunkPreviewLines = @($sqlChunks | Select-Object -First 15)

$sqlLike = (cmd /c "docker exec ragchatbot-mysql mysql -uroot -proot ragchatbot -N -e `"SELECT COUNT(*) FROM document_chunks WHERE document_id = UNHEX('$docBin') AND deleted_at IS NULL AND (content LIKE '%LichSuPhucKhao%' OR content LIKE '%BienBan%');`" 2>&1").Trim()
$sqlLikeStr = if ($sqlLike -is [array]) { ($sqlLike | Where-Object { $_ -match '^\d+$' } | Select-Object -First 1) } else { "$sqlLike" }
if ($sqlLikeStr -match "^\d+$") { $results.sqlChunksWithPage14Entities = [int]$sqlLikeStr.Trim() } else { $results.sqlChunksWithPage14Entities = $sqlLikeStr }

# Qdrant scroll (filter widget + document)
$scrollBody = @{
    filter = @{
        must = @(
...
# Target chat (UTF-8 bytes — avoid .ps1 encoding mojibake)
...
$smoke += @{ id = "fact"; answerPreview = $fact.answer.Substring(0, [Math]::Min(200, $fact.answer.Length)); pass = ($fact.answer -match "22T1020585"); sourceCount = $fact.sources.Count }
...
$smoke += @{ id = "table"; answerPreview = $table.answer.Substring(0, [Math]::Min(250, $table.answer.Length)); colHits = $colHit; pass = ($colHit -ge 6); sourceCount = $table.sources.Count }
...
$smoke += @{ id = "oos"; answerPreview = $oos.answer.Substring(0, [Math]::Min(200, $oos.answer.Length)); pass = $oosPass; sourceCount = $oos.sources.Count }
...
Write-Host "Parser merge seen: $($results.parserMergeSeen)"
