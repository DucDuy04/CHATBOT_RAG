...
public class EmbeddingService {
    ...
    private void upsertToQdrant(List<Embedding> embeddings, List<TextSegment> segments, UUID documentId) {
        ...
        for (int i = 0; i < embeddings.size(); i++) {
            ...
            Map<String, Object> payload = buildQdrantRestPayload(seg);
    ...
    /**
     * Builds one Qdrant REST upsert payload map (UTF-8 safe when serialized via Jackson/RestClient).
     * Package-visible for regression tests (task 24D4 / 25A2).
     */
    static Map<String, Object> buildQdrantRestPayload(TextSegment segment) {
        Map<String, Object> payload = new LinkedHashMap<>();
        payload.put("text_segment", segment.text());
